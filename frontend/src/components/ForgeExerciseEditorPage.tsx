...
import { addForgeSessionExercise, createForgeExercise, generateForgeSessionExerciseAdditionCoaching, getForgeExercises, getForgeMachineProfiles, updateForgeExercise } from '../api/api';
...
export default function ForgeExerciseEditorPage() {
  ...
  const [loading, setLoading] = useState(Boolean(editingId)); const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null);
  ...
  return <div className="space-y-4 forge-anim"><header className="flex items-start gap-3"><button onClick={() => navigate(returnTo || '/forge/exercises')} className="tap mt-1 flex h-9 w-9 items-center justify-center rounded-full" aria-label="Zurück zur Übungsbibliothek" style={{ color: SAND, background: 'rgba(232,197,138,0.08)' }}><ArrowLeft size={18} /></button><div><p className="text-[10px] uppercase tracking-[0.18em]" style={{ color: SAND }}>Übungsbibliothek</p><h1 className="mt-1 text-[25px] font-semibold" style={{ color: TEXT }}>{editingId ? 'Übung bearbeiten' : 'Neue Übung'}</h1><p className="mt-1 text-[11px]" style={{ color: DIM }}>Bewegung einmal anlegen und in allen Trainingstagen nutzen.</p></div></header>
    ...
    {machineCapable && <section className="card-forge p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.16em]" style={{ color: SAND }}>Maschinenprofile</p><h2 className="mt-1 text-[15px] font-semibold" style={{ color: TEXT }}>Bestehende Profile zuweisen</h2><p className="mt-1 text-[10px]" style={{ color: DIM }}>Profile bleiben zentral und werden nicht dupliziert.</p></div><button onClick={() => { sessionStorage.setItem(EDITOR_DRAFT_KEY, JSON.stringify({ path: window.location.pathname + window.location.search, draft })); navigate(`/forge/machine-profiles?returnTo=${encodeURIComponent(window.location.pathname + window.location.search)}`); }} className="tap p-2" style={{ color: SAND }} aria-label="Maschinenprofile verwalten"><Settings2 size={17} /></button></div><div className="mt-3 space-y-2">{profiles.map((profile) => { const selected = draft.machine_profile_ids.includes(profile.id); return <label key={profile.id} className="flex items-center gap-3 rounded-xl p-3" style={{ border: `1px solid ${selected ? `${SAND}66` : BORDER}`, background: selected ? 'rgba(232,197,138,0.07)' : 'rgba(255,247,235,0.02)' }}><input type="checkbox" checked={selected} onChange={() => toggleProfile(profile.id)} className="accent-[#e8c58a]" /><span className="min-w-0 flex-1"><span className="block text-[12px] font-medium" style={{ color: TEXT }}>{profile.name}</span><span className="block truncate text-[10px]" style={{ color: DIM }}>{profile.model || 'Ohne Modell'}{profile.notes ? ` · ${profile.notes}` : ''}</span></span>{selected && <Check size={15} style={{ color: SAND }} />}</label>; })}{!profiles.length && <div className="rounded-xl p-4 text-center text-[11px]" style={{ color: DIM, border: `1px dashed ${BORDER}` }}>Noch keine Maschinenprofile. Lege zentral das erste an.</div>}</div></section>}
