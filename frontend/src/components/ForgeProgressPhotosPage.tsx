...
function PhotoCard({ photo, imageUrl, selected, saving, onCompare, onEdit, onDelete }: { photo: ForgeProgressPhoto; imageUrl?: string; selected: boolean; saving: boolean; onCompare: () => void; onEdit: () => void; onDelete: () => void }) {
  const context = [photo.context.weight_kg != null ? `${photo.context.weight_kg.toFixed(1)} kg` : null, photo.context.current_goal, photo.context.workout_names.length ? photo.context.workout_names.join(' · ') : null].filter(Boolean).join(' · ');
