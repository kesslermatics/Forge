  ...
  useEffect(() => {
    Promise.all([getForgePlans(), getForgeExercises()]).then(([plans, library]) => {
      ...
      if (editing) {
        ...
        if (parsed?.path === location.pathname) { setDraft(parsed.draft); sessionStorage.removeItem(DRAFT_KEY); }
        else setDraft(loaded ? toInput(loaded) : { name: '', description: '', plan_type: 'workout', default_duration_minutes: null, position: plans.length, exercises: [] });
      }
