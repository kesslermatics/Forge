...
const translations = {
    ...
    'settings.apiKeysTitle': { de: 'API-Schlüssel & MCP', en: 'API keys & MCP' },
    'settings.apiKeysDesc': { de: 'Erstelle einen dauerhaft gültigen Schlüssel für deinen persönlichen Forge-MCP-Server. Er gilt bis zum Widerruf und erlaubt nur lesenden Zugriff auf die Coach-Tools.', en: 'Create a non-expiring key for your personal Forge MCP server. It remains valid until revoked and only grants read access to Coach tools.' },
    'settings.apiKeyName': { de: 'Name des Schlüssels', en: 'Key name' },
    'settings.apiKeyNamePlaceholder': { de: 'z. B. Mein Agenten-Chat', en: 'e.g. My agent chat' },
    'settings.apiKeyCreate': { de: 'API-Schlüssel erstellen', en: 'Create API key' },
    'settings.apiKeyCreating': { de: 'Wird erstellt…', en: 'Creating…' },
    'settings.apiKeysLoading': { de: 'API-Schlüssel werden geladen…', en: 'Loading API keys…' },
    'settings.apiKeysEmpty': { de: 'Noch keine API-Schlüssel erstellt.', en: 'No API keys created yet.' },
    'settings.apiKeyCreated': { de: 'API-Schlüssel wurde erstellt.', en: 'API key created.' },
    'settings.apiKeyCreatedTitle': { de: 'API-Schlüssel jetzt kopieren', en: 'Copy your API key now' },
    'settings.apiKeyCreatedDesc': { de: 'Dieser Schlüssel wird nur einmal angezeigt und kann später nicht wiederhergestellt werden. Bewahre ihn wie ein Passwort auf.', en: 'This key is shown only once and cannot be recovered later. Store it like a password.' },
    'settings.apiKeyCopy': { de: 'Kopieren', en: 'Copy' },
    'settings.apiKeyCopied': { de: 'Kopiert', en: 'Copied' },
    'settings.apiKeyCopyFailed': { de: 'Automatisches Kopieren fehlgeschlagen. Markiere den Schlüssel und kopiere ihn manuell.', en: 'Automatic copy failed. Select the key and copy it manually.' },
    'settings.apiKeyDone': { de: 'Fertig', en: 'Done' },
    'settings.apiKeyClose': { de: 'Dialog schließen', en: 'Close dialog' },
    'settings.apiKeyNeverExpires': { de: 'Läuft nicht ab', en: 'Never expires' },
    'settings.apiKeyLastUsed': { de: 'Zuletzt verwendet', en: 'Last used' },
    'settings.apiKeyNeverUsed': { de: 'Noch nie verwendet', en: 'Never used' },
    'settings.apiKeyRevoked': { de: 'Widerrufen', en: 'Revoked' },
    'settings.apiKeyRevoke': { de: 'Widerrufen', en: 'Revoke' },
    'settings.apiKeyRevokeTitle': { de: 'API-Schlüssel widerrufen?', en: 'Revoke API key?' },
    'settings.apiKeyRevokeDesc': { de: '„{name}“ verliert sofort den Zugriff auf deine Coach-Daten. Dieser Vorgang kann nicht rückgängig gemacht werden.', en: '“{name}” immediately loses access to your Coach data. This action cannot be undone.' },
    'settings.apiKeyRevokedSuccess': { de: 'API-Schlüssel wurde widerrufen.', en: 'API key revoked.' },
    'settings.apiKeyLoadFailed': { de: 'API-Schlüssel konnten nicht geladen werden.', en: 'API keys could not be loaded.' },
    'settings.apiKeyCreateFailed': { de: 'API-Schlüssel konnte nicht erstellt werden.', en: 'API key could not be created.' },
    'settings.apiKeyRevokeFailed': { de: 'API-Schlüssel konnte nicht widerrufen werden.', en: 'API key could not be revoked.' },
